...
class AutoChecklistAgent:
    ...
    def _check_duration_anomaly(self, norm: str, resume: ResumeMedis) -> Dict:
        """Check for duration anomalies using ML or rules. Duration is INCLUSIVE."""
        ...
        # Calculate duration INCLUSIVELY: (end - start).days + 1
        ...
        if is_normal:
            ...
            mid = (expected_range['min'] + expected_range['max']) / 2
            diff = abs(duration - mid)
            if diff <= 1:
                score = 70
            elif diff <= 3:
                score = 50
            ...
            return {
                ...
                "status": status,
                "score": score,
                "message": f"Durasi rawat {duration} hari di luar ekspektasi",
                "details": {
                    "duration_days": duration,
                    "expected_range": f"{expected_range['min']}-{expected_range['max']} hari",
                    "severity": severity,
    ...
    def _check_cppt_frequency(self, norm: str, resume: ResumeMedis) -> Dict:
        """Check if CPPT is recorded frequently enough. Duration is INCLUSIVE."""
        if not resume.tanggal_masuk or not resume.tanggal_keluar:
            return {
    ...
    def _extract_duration_features(self, norm: str, resume: ResumeMedis,
        ...
        features = [
            duration,
            len(resume.diagnosa_utama or ''),
            1 if resume.diagnosa_penyerta else 0,
            1 if resume.tindakan else 0,
            self.db.query(CPPT).filter(CPPT.norm == norm).count(),
            self.db.query(Penunjang).filter(Penunjang.norm == norm).count(),
            self.db.query(LaporanOperasi).filter(LaporanOperasi.norm == norm).count(),
        ]
        return features
